(function (jf, undefined){
    ...
    jf.parse = function(statblock){
        ...
        //Clean text        
        statblock = unescape(statblock).replace(/<br[^>]*>/g, '#').replace(/(<([^>]+)>)/ig,"");
    ...
    function parseActions(){
        ...
        _.each(actions, function(value, key){
            ...
            if(pos != -1) {
                jf.setAttribut(monster.id, 'npc_action_description' + cpt, value.substring(0, pos).replace(/\.#/g,'.\n').replace(/#/g,' '));
                jf.setAttribut(monster.id, 'npc_action_effect' + cpt, value.substring(pos).replace(/\.#/g,'.\n').replace(/#/g,' '));
            }
            else {
                jf.setAttribut(monster.id, 'npc_action_description' + cpt, value.replace(/\.#/g,'.\n').replace(/#/g,' '));
            }
...
//function findObjs(filter){ return [obj];}
