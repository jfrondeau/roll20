...
var Parser = function() {
    ...
    this.parse = function(statblock) {
        ...
        //Clean text        
        this.statblock = unescape(statblock).replace(/<br[^>]*>/g, '#').replace(/(<([^>]+)>)/ig,"");
    ...
    this.splitSection = function(statblock) {
        ...
        // Check for bio (flavor text) separated by 3 line breaks
        if((pos = statblock.indexOf('###')) != -1){
            bio = statblock.substring(pos).replace(/#{2,}/g, '').replace(/\.#/g,'.\n').replace(/#/g,' ');
            statblock = statblock.substring(0,pos);
        }

        // Cherche l'index du début du trait
        var match = statblock.match(/#(?!armor class|hit points|speed|str|dex|con|int|wis|cha|saving throws|skills|damage resistances|damage immunities|condition immunities|damage vulnerabilities|senses|languages|challenge)((?:[A-Z][a-z]+[0-9-\s(\/)]{0,5})+)\./i);
        ...
        if(match !== null)
        {
            ...
            // reste du statblock
            traits = splitRegex(powers, /#\s*(actions|legendary actions)(?:\s|#)/gmi);
        ...
        for (var attrname in traits) { section[attrname] = traits[attrname]; } // Combine le tout.
    ...
    this.parseChallenge = function() {
        ...
        var match = input.match(/([\d/]+).*?(\d+)/);
        this.setAttribut(this.npc.id, 'npc_challenge', match[1]);
